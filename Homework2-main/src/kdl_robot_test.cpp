...
int main(int argc, char **argv)
{
    ...
    ros::Publisher error_pub = n.advertise<std_msgs::Float64>("error", 1);      
    ...
    // Messages
    std_msgs::Float64 tau1_msg, tau2_msg, tau3_msg, tau4_msg, tau5_msg, tau6_msg, tau7_msg, err_msg;
    ...
    KDLPlanner planner(traj_duration,0.10, acc_duration, init_position, end_position);  
    ...
    // Traj choice <2 circular >=2 rectilinear;
    int traj_type=3;
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            {
                std::cout << "rotazione desiderat: " << des_pose.M << std::endl;
