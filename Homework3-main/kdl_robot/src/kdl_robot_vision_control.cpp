...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        if (robot_state_available && aruco_pose_available)
        {
            ...
            // Eigen::Matrix<double,3,3> Rot_des_ZYX = toEigen(KDL::Rotation::EulerZYX((-90.0+desired_orient[0])*toRadians, desired_orient[1]*toRadians, (-90.0+desired_orient[2])*toRadians));
            
            // // desired position computation
            // Eigen::Vector3d offset(0.0, 0.0, 0.5);
            // Eigen::Matrix<double,3,1> p_cam_object = toEigen(robot.getEEFrame().M)*toEigen(cam_T_object.p);
            // Eigen::Matrix<double,3,1> p_bas_object = toEigen(robot.getEEFrame().p) + p_cam_object;
            ...
            // // HW.2a control law fixed position and orientation offset
            // Eigen::Matrix<double,3,1> e_rot_ee_offset = computeOrientationError(Rot_des_ZYX, toEigen(robot.getEEFrame().M));
            // Eigen::Matrix<double,3,1> e_p_ee_offset = computeLinearError(p_e_des_offset, toEigen(robot.getEEFrame().p));  
            // Eigen::Matrix<double,6,1> x_tilde = Eigen::Matrix<double,6,1>::Zero(); 
            // x_tilde << e_p_ee_offset,  e_rot_ee_offset[0], e_rot_ee_offset[1], e_rot_ee_offset[2];                                  
            // dqd.data = lambda*J_pinv*x_tilde+ 10*(Eigen::Matrix<double,7,7>::Identity() - J_pinv*J_cam.data)*(qdi - toEigen(jnt_pos));
            ...
            // HW.2b control law 
            dqd.data = 20*LJ_pinv*Eigen::Vector3d(0,0,1)+ 10*N*(qdi - toEigen(jnt_pos));
