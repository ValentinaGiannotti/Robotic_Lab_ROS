...
TF_NAV::TF_NAV() {
    ...
    orientation_quat.setRPY(0.0, 0.0, -90.0);
    _home_or << orientation_quat.x(), orientation_quat.y(), orientation_quat.z(), orientation_quat.w();
}
...
void TF_NAV::tf_listener_fun() {
    ...
    while(ros::ok()){
        ...
        _cur_or << transform.getRotation().x(), transform.getRotation().y(), transform.getRotation().z(), transform.getRotation().w();
        position_pub();
        r.sleep();
    }
}
...
void TF_NAV::position_pub() {
    ...
    pose.pose.orientation.x = _cur_or[0];
    pose.pose.orientation.y = _cur_or[1];
    pose.pose.orientation.z = _cur_or[2];
    pose.pose.orientation.w = _cur_or[3];

    ROS_DEBUG("Current position:\t %f, %f, %f", _cur_pos[0], _cur_pos[1], _cur_pos[2]);
    ROS_DEBUG("Current orientation [x,y,z,w]:\t %f, %f, %f, %f\n", _cur_or[0], _cur_or[1], _cur_or[2], _cur_or[3]);
